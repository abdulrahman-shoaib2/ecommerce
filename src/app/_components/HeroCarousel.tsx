  ...
  return (
    <>
      <div className="  overflow-hidden bg-[url(../assets/media/home-slider-1.png)] bg-cover bg-center">
        <CarouselComponent setApi={setCarouselApi} opts={{ loop: true,  }} plugins={[autoplay.current]} onMouseEnter={autoplay.current.stop} onMouseLeave={() => { autoplay.current.reset(); autoplay.current.play() }} className='bg-linear-to-r from-green-500/90 to-green-400/50'>
          <CarouselContent className="relative w-[105%] ease-in-out ">
            {slides.map((slide, index) => (
              <CarouselItem key={'slide_'+index} className='bg-transparent ease-in-out'>
                <CustomSlide slide={slide} current={current} index={index} />
              </CarouselItem>
            ))}
          </CarouselContent>
