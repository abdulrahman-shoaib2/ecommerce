...
import { footerLinks, paymentMethods, socialMediaIcons } from "@/constants/footer";
            ...
            <div className="space-y-3 mb-6">
              {
                contactInfo.map((info: IContactInfoItem, index: number) => {
                  return (
                    ...
                    >
                      <IconText icon={info.icon.faName} text={info.text?.content} classNames={{ icon: info.icon.className, }} />
            ...
            <div className="flex items-center gap-3">
              {
                socialMediaIcons.map((icon: IContactInfoItem, index: number) => {
                  return (
                    ...
                    >
                      <Icon icon={icon.icon.faName} classNames={{ icon: icon.icon.className }} />
            ...
            <div className="flex items-center gap-4">
              {
                paymentMethods.map((method, index) => {
                  return (
                    <IconText icon={method.icon.faName} classNames={{ icon: method.icon.className, innerWrapper: 'flex items-center gap-2 text-gray-500 text-sm' }} text={method.name} key={method.name + '_' + index} />
