...
export default function OfferCard({ badge, title, description, discount, code, link, color, btnText="Shop Now"  }: { badge: {icon:string,text:string}, title: string, description: string, discount: string, code: string, link: string, color: {from:string,to:string, btnIcon:string, };btnText?:string }) {
  return (

    <div className={`relative overflow-hidden rounded-2xl bg-linear-to-br ${color.from} ${color.to} p-8 text-white`}>
